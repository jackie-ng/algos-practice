class Solution:
    def maxEvents(self, events: List[List[int]]) -> int:
        ...
        count = 0
        while i < n or hp:
            # No day exists in heap, pick current starting day
            if not hp:
                cur_day = sorted_events[i][0]
            # Iterate over events that have same starting day as current day, and add their end days to min heap
            ...
            cur_day += 1
            # If current day has exceeded any event end time, we cant attend it => Pop it
